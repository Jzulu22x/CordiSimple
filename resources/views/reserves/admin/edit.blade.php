    ...
    <div class="flex justify-center items-center min-h-screen">
        <div class="max-w-[720px] mx-auto">
            <div class="relative flex flex-col text-gray-700 bg-white shadow-md w-96 rounded-xl bg-clip-border">
                ...
                <form action="{{ route('reserves.update', $reserveEdit->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="flex flex-col gap-4 p-6">
                        ...
                        <div class="relative h-11 w-full min-w-[200px]">
                            <select
                                ...
                                name="event_id" placeholder=" " required>
                
                                <option value="{{$reserveEdit->event->id}}" selected>{{ $reserveEdit->event->name }}</option>
                                @foreach ($events as $event)
                                    @if ($event->id != $reserveEdit->event->id)
                        ...
                        <div class="relative h-11 w-full min-w-[200px]">
                            <select
                                class="outline-none w-full h-full px-3 py-3 font-sans text-sm font-normal transition-all bg-transparent border rounded-md peer border-blue-gray-200 border-t-transparent text-blue-gray-700 outline outline-0 placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 focus:border-2 focus:border-gray-900 focus:border-t-transparent focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50"
                                name="user_id" placeholder=" " required>
                                <option value="{{$reserveEdit->user->id}}" selected>{{ $reserveEdit->user->name}}</option>
                                @foreach ($users as $user)
                                    @if ($user->id != $reserveEdit->user->id)
                        ...
                        <div class="relative h-11 w-full min-w-[200px]">
                            <select
                                class="outline-none w-full h-full px-3 py-3 font-sans text-sm font-normal transition-all bg-transparent border rounded-md peer border-blue-gray-200 border-t-transparent text-blue-gray-700 outline outline-0 placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 focus:border-2 focus:border-gray-900 focus:border-t-transparent focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50"
                                name="status_id" placeholder=" " required>
                                <option value="{{$reserveEdit->status->id}}" selected>{{$reserveEdit->status->name}}</option>
                                @foreach ($statuses as $status)
                                    @if ($status->id != $reserveEdit->status->id)
